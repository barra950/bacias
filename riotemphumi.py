...
plt.pcolormesh(lon, lat ,temp[0],transform = ccrs.PlateCarree(),color='k',cmap=cmap)
...
for k in range(0,len(casos)):
    for t in range(0,3):
        #classification_result_humi = classify_distribution(weekmeanhumi)
        #classification_result_temp = classify_distribution(weekmeantemp)
        ...
        ds2 = shape_clip_dataarray(data_xr, shapefile_br, state_index=0, invert=True, all_touched=False)
        mask = ds2.T    
        ...
        classificacao_casos = np.zeros((55, 1))
        estados_total = np.zeros((91, 1))

        for kk in temphumi:
            for tt in kk:
                if tt == "23":
                    classificacao_casos[int(kk)] = casos[k,t]


print(temphumi.shape)
